...
export default function Testimonials() {
    ...
    return (
        <section className="pt-16 pb-0 bg-gray-50">
            ...
            {/* Bandeau défilant — fond blanc, cartes grises */}
            <div className="w-full overflow-hidden relative py-6 bg-gray-50">
                {/* Fade gauche */}
                ...
                <div className="flex gap-4 w-max" style={{ animation: "marquee 32s linear infinite" }}>
                    {doubled.map((r, i) => (
                        <div key={i} className="flex items-start gap-3 bg-white border border-gray-100 rounded-xl p-4 w-72 flex-shrink-0 shadow-sm">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 bg-red-600">
                                {r.name[0]}
                            </div>
                            <div>
                                <StarRow />
                                <p className="text-xs font-semibold text-gray-900 mb-1">{r.name}</p>
